...
class MjViewer(MjViewerBasic):
    ...
    def quatToRPY(self, quat):
        ...
        return ['%.2f' %np.rad2deg(roll_Y), '%.2f' %np.rad2deg(pitch_X), '%.2f' %np.rad2deg(yaw_z)]

    def print2dp(self, list3):
    ...
    def _create_full_overlay(self):
        ...
        self.add_overlay(const.GRID_BOTTOMLEFT, "timestep", "%.5f" % self.sim.model.opt.timestep)
        self.add_overlay(const.GRID_BOTTOMLEFT, "SIM DATA TIME", "%.5f" % self.sim.data.time)
        self.add_overlay(const.GRID_BOTTOMLEFT, "n_substeps", str(self.sim.nsubsteps))
        self.add_overlay(const.GRID_TOPLEFT, "Toggle geomgroup visibility", "0-4")
        
        self.add_overlay(const.GRID_TOPRIGHT, "WAIST[1] XYZ", str(self.print2dp(self.sim.data.body_xpos[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "WAIST[1] RPY", str(self.quatToRPY(self.sim.data.body_xquat[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "NECK [8] RPY", str(self.quatToRPY(self.sim.data.body_xquat[8])))
        self.add_overlay(const.GRID_TOPRIGHT, "------- FEET -------", "------------------------")
        self.add_overlay(const.GRID_TOPRIGHT, "L FOOT RPY", str(self.quatToRPY(self.sim.data.body_xquat[36])))
        self.add_overlay(const.GRID_TOPRIGHT, "L FOOT XYZ", str(self.print2dp(self.sim.data.body_xpos[36] - self.sim.data.body_xpos[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "R FOOT RPY", str(self.quatToRPY(self.sim.data.body_xquat[47])))
        self.add_overlay(const.GRID_TOPRIGHT, "------- HANDZ -------", "------------------------")
        self.add_overlay(const.GRID_TOPRIGHT, "L HAND RPY", str(self.quatToRPY(self.sim.data.body_xquat[17])))
        self.add_overlay(const.GRID_TOPRIGHT, "L HAND XYZ", str(self.print2dp(self.sim.data.body_xpos[17] - self.sim.data.body_xpos[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "R HAND RPY", str(self.quatToRPY(self.sim.data.body_xquat[25])))
        self.add_overlay(const.GRID_TOPRIGHT, "L HAND XYZ", str(self.print2dp(self.sim.data.body_xpos[17] - self.sim.data.body_xpos[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "R HAND XYZ", str(self.print2dp(self.sim.data.body_xpos[25] - self.sim.data.body_xpos[1])))
        self.add_overlay(const.GRID_TOPRIGHT, "------ ze BALL ------", "------------------------")
        self.add_overlay(const.GRID_TOPRIGHT, "BALL XYZ", str(self.print2dp(self.sim.data.body_xpos[48])))
        # self.add_overlay(const.GRID_TOPRIGHT, "HAND BALL ", str(self.print2dp(self.sim.data.body_xpos[50] + [0.5, 1, 1])))

        # self.add_overlay(const.GRID_TOPRIGHT, "------- SENSORS -------", "------------------------")
        # self.add_overlay(const.GRID_TOPRIGHT, "ACCELERO     ", str(self.print2dp(self.sim.data.sensordata[:3])))
        # self.add_overlay(const.GRID_TOPRIGHT, "GYRO GRYRO   ", str(self.print2dp(self.sim.data.sensordata[2:5])))
        # self.add_overlay(const.GRID_TOPRIGHT, "LF FORCE     ", str(self.print2dp(self.sim.data.sensordata[6:9])))
        # self.add_overlay(const.GRID_TOPRIGHT, "RF FORCE     ", str(self.print2dp(self.sim.data.sensordata[12:15])))
        # self.add_overlay(const.GRID_TOPRIGHT, "LF MOM     ", str(self.print2dp(self.sim.data.sensordata[9:12])))
        # self.add_overlay(const.GRID_TOPRIGHT, "RF MOM     ", str(self.print2dp(self.sim.data.sensordata[9:12])))

        self.add_overlay(const.GRID_BOTTOMRIGHT, "------- PARA PARA -------", "------------------------")
        self.add_overlay(const.GRID_BOTTOMRIGHT, "CATCHIN' BODIES!!  ", str('%.2f' %(sum(self.sim.model.body_mass[1:48]))))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "BALL MASS ", str('%.2f' %(sum(self.sim.model.body_mass[48:]))))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "FLOOR FRICTION[0] ", str(self.print2dp(self.sim.model.geom_friction[0])))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "BALL FRICTION[73] ", str(self.print2dp(self.sim.model.geom_friction[73])))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "DOF FRICTION LOSS", str('%.2f' %self.sim.model.dof_frictionloss[6]))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "DOF DAMPING[6:38]", str('%.2f' %self.sim.model.dof_damping[6]))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "JOINT STIFFNESS[1]", str('%.2f' %self.sim.model.jnt_stiffness[1]))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "GRAVITY", str(self.print2dp(self.sim.model.opt.gravity)))
        self.add_overlay(const.GRID_BOTTOMRIGHT, "WIND",    str(self.sim.model.opt.wind))
